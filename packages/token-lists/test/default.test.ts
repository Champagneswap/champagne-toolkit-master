...
import currentChampagneswapDefaulList from "../lists/champagneswap-default.json";
import currentChampagneswapExtendedList from "../lists/champagneswap-extended.json";
import currentChampagneswapTop100List from "../lists/champagneswap-top-15.json";
import currentChampagneswapTop15List from "../lists/champagneswap-top-100.json";
import { buildList, VersionBump } from "../src/buildList";

const currentLists = {
  "champagneswap-default": currentChampagneswapDefaulList,
  "champagneswap-extended": currentChampagneswapExtendedList,
  "champagneswap-top-100": currentChampagneswapTop100List,
  "champagneswap-top-15": currentChampagneswapTop15List,
};
...
expect.extend({
  ...
  toBeValidLogo(token) {
    ...
    const hasTWLogo =
      token.logoURI === `https://tokens.champagneswap.com/images/${token.address}.png`;
    let hasLocalLogo = false;
    const refersToLocalLogo = token.logoURI === `https://tokens.champagneswap.com/images/${token.address}.png`;
...
describe.each([["champagneswap-default"], ["champagneswap-extended"], ["champagneswap-top-100"], ["champagneswap-top-15"]])(
  "buildList %s",
  (listName) => {
