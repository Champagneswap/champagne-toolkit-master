# ChampagneSwap Token Lists

This repo contains main ChampagneSwap token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to ChampagneSwap default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to ChampagneSwap (extended) token list
...
- Add new tokens to `src/tokens/champagneSwap-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on ChampagneSwap v2, and update list.
$ yarn fetch:pcs-top-100

# Build token list (ChampagneSwap-top-100.json)
...
Netlify simply takes the json files under `lists` directory and hosts them on `tokens.ChampagneSwap.finance/list-name.json`
